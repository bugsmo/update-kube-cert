...
- subj
   - 原来是从旧crt文件获取，修改为固定主机名 `  subj="/CN=hz-kubesphere-master03"`
```shell
...
check_file "${cert}"
# need # subj=$(openssl x509 -text -noout -in "peer.crt" | grep "Subject:" | sed 's/Subject:/\//g;s/\,/\//;s/[[:space:]]//g')
...
- subject_alt_name
   - 原来是从旧crt文件获取，修改为固定IP `alt_name="DNS:hz-kubesphere-master03, DNS:localhost, IP:10.160.103.103, IP:127.0.0.1, IP:0:0:0:0:0:0:0:1"`
```shell
...
check_file "${cert}" # 检查文件是否存在
# need # alt_name=$(openssl x509 -text -noout -in "peer.crt" | grep -A1 'Alternative' | tail -n1 | sed 's/[[:space:]]*Address//g')
...
cert::get_subject_alt_name() {
  ...
  check_file "${cert}"
  #alt_name=$(openssl x509 -text -noout -in "${cert}" | grep -A1 'Alternative' | tail -n1 | sed 's/[[:space:]]*Address//g')
  alt_name="DNS:hz-kubesphere-master03, DNS:localhost, IP:10.160.103.103, IP:127.0.0.1, IP:0:0:0:0:0:0:0:1"
  printf "%s\n" "${alt_name}"
}
...
cert::get_subj() {
  ...
  check_file "${cert}"
  #subj=$(openssl x509 -text -noout -in "${cert}" | grep "Subject:" | sed 's/Subject:/\//g;s/\,/\//;s/[[:space:]]//g')
  subj="/CN=hz-kubesphere-master03"
  printf "%s\n" "${subj}"
}
